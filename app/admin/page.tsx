...
// Dynamically import Leaflet components with SSR disabled to prevent "window is not defined"
const MapContainer = dynamic(() => import("react-leaflet").then((mod) => mod.MapContainer), { ssr: false });
const TileLayer = dynamic(() => import("react-leaflet").then((mod) => mod.TileLayer), { ssr: false });
const Marker = dynamic(() => import("react-leaflet").then((mod) => mod.Marker), { ssr: false });
const Popup = dynamic(() => import("react-leaflet").then((mod) => mod.Popup), { ssr: false });

// MapController lives in a separate client-only component to safely use useMap hook
const MapController = dynamic(() => import("@/components/AdminMapController"), { ssr: false });
...
export default function AdminPage() {
  ...
  // Auth guard: require both a Supabase session and admin role
  useEffect(() => {
    const checkAuth = async () => {
      const { data: { session } } = await supabase.auth.getSession();
      const role = sessionStorage.getItem("chitti_role");
  ...
  // Initialise Leaflet icons (client-only) once authorised
  useEffect(() => {
  ...
  // Show loading / redirect screen during auth check
  if (isCheckingAuth || !isAuthorized) {
    return (
      <div className="flex h-screen w-full items-center justify-center bg-[#050b18] text-white">
  ...
  return (
    <div className="flex h-screen w-full bg-[#050b18] text-white overflow-hidden font-body">
        ...
        <div className="p-4 border-b border-white/10 flex items-center justify-between bg-navy-900/60 backdrop-blur-md">
          <div className="flex items-center gap-3">
            <div className="w-9 h-9 rounded-xl bg-white/5 border border-white/10 flex items-center justify-center p-1.5 shrink-0 shadow-md">
              <img src="/chitti_mother_square-min.png" alt="Chitti Logo" className="w-full h-full object-contain" />
            </div>
            <div>
      ...
      <div className="flex-1 h-full relative">
        <MapContainer center={CHENNAI_CENTER} zoom={15} zoomControl={false} className="h-full w-full">
          <TileLayer
            url="https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png"
            className="invert-[100%] hue-rotate-[180deg] brightness-[80%] contrast-[120%]"
          />
          {/* MapController uses flyTo instead of remounting the container */}
